...
namespace TPCWC{
	...
	public class InputManager : MonoBehaviour {
		...
		void FixedUpdate() {
		//	if(GetComponent<Movement>() != null && GetComponent<Movement>().enabled)
			//	return;
				//Setting input
			AllInputs();
		...
		void UpdateCharacterMotor(){
			...
			if(moveJoy != null){
				...
				if(myMove.x > 0.1f || myMove.x < -0.1f){
					horizontal = myMove.x;
				}
			}
			...
			if(FindObjectOfType<GameController>().android){
				//caching the vert direction with forward direction 
				vert = vertical * Camera.main.transform.forward;
				//caching the horz direction with right direction 
				horz = horizontal * Camera.main.transform.right;
			}else{
				vert = vertical * cam.transform.forward;
				//caching the horz direction with right direction 
				horz = horizontal * cam.transform.right;
			}
