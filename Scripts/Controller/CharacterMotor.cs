...
public class CharacterMotor : MonoBehaviour {
		...
		public void InitializeCharacterMotor(){
			...
			if(FindObjectOfType<CombatManager>() == null)
				transform.position = GetComponent<PersonnageDataManager>().perso.position;
		...
		/*	rigidBody.constraints = RigidbodyConstraints.FreezeRotationX | 
			...
			if(modelMesh == null)
				...
				animator = modelMesh.GetComponentInChildren<Animator>();
		...
		public void FixedUpdate(){
			...
			if(GetComponent<ActionManager>() && GetComponent<ActionManager>().GetWait())
				return; 

			if(GetComponent<PersonnageDataManager>() && GetComponent<PersonnageDataManager>().isDie)
				return; 
			...
			if(!movement && !GetComponent<ActionManager>().wait)
		...
		void SavePosition(){
			if(animator.GetFloat("vertical") > 0.5f && FindObjectOfType<CombatManager>() == null)
				GetComponent<PersonnageDataManager>().perso.position = transform.position;
		}
